...
- Projects: pay application **invoice #** in the workspace header is persisted on **Save Project** as `project.pay_application_invoice_number` (optional text). Migration: `20260405200000_project_pay_application_invoice_number` (includes `README.md` with manual rollback SQL). Contract spec updated (R8). `ProjectFormState` lives in `projects.models.ts`; API mapping and save-payload helpers tested under `tests/app/projects/`.
- **Repo hygiene:** stop tracking `tsconfig.tsbuildinfo` (listed in `.gitignore`).
- Pay application **Phases** tab: restore the prior Phase 1 / Phase 2 layout (locate ticket, dates, onsite review, surveyed, status, nested pay line rows with quantity + installed). Contract tab unchanged. Phase rows are seeded from current contract pay lines and reset when pay lines change; phase-only edits remain local (not part of project save).
