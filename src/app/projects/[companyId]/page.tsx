...
export default async function CompanyProjectsPage({ params }: PageProps) {
  ...
  const { companies, projects: allProjects } = await fetchProjects()
  const company = companies.find((item) => item.id === companyId);
  if (!company) notFound();

  const projects = allProjects.filter((project) => project.companyId === company.id);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          <Link
            href={`/projects/new?companyId=${encodeURIComponent(company.id)}`}
