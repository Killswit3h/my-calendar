...
const columns: TableColumn<AdminFixture>[] = [
  { key: 'id', header: 'User', width: 140 },
  { key: 'name', header: 'Name', width: 220 },
  { key: 'status', header: 'Status', width: 140 },
  { key: 'updatedAt', header: 'Last Updated', width: 160 },
]

export default function AdminUsersPage() {
  const rows = ADMIN_FIXTURES.filter(item => item.type === 'User')
  return (
    <div className="space-y-4">
      <Toolbar>
        <div className="text-sm text-muted">{rows.length} active users</div>
      ...
      <DataTable
        ...
        emptyMessage={
          <EmptyState
            title="No users"
            description="Invite coworkers to grant access."
