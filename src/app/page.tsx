...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative grid gap-6 px-6 py-6 md:grid-cols-[minmax(0,1fr)_320px] md:items-start">
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted">
              Calendar remains your source of truth. The control center spots the trends and surfaces next actions—one click
              jumps right back into scheduling.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link
                href={CALENDAR_HOME_PATH}
                className="inline-flex items-center gap-2 rounded-full bg-[var(--gfc-green)] px-5 py-2 text-sm font-semibold text-white shadow-card transition hover:scale-[1.02]"
              >
      ...
      <Card tone="glass" className="h-[360px] space-y-4">
        <header className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-[0.28em] text-muted">Map • Weather • Conflicts</h2>
            <p className="text-xs text-muted">Layer traffic, outages, and weather alerts against staging yards.</p>
          </div>
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-border/60 text-muted">
            <Compass className="h-4 w-4" />
          </span>
        </header>
        <div className="flex h-[240px] items-center justify-center rounded-2xl border border-dashed border-border/50 bg-surface-soft/70 text-sm text-muted">
          GIS and live telemetry coming soon—connect Esri, Google, or Samsara layers.
        </div>
        <div className="text-xs text-muted">
          <strong>Status:</strong> Integrations in progress
        </div>
      </Card>
    </div>
  )
