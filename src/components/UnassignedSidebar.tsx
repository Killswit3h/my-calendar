...
export default function UnassignedSidebar({ employees, events, anyDateInView = null, selectedDate = null, weekStartsOn = 1, onQuickAdd, onSetYard }: Props) {
  ...
  const assignedByDay = useMemo(() => {
    ...
    if (!daysToShow.length || !events?.length) return map;
  ...
  return (
    <div className="unassigned-sidebar" aria-label="Unassigned employees by day">
              ...
              <div key={key} className="day-card" role="listitem" aria-label={`Unassigned for ${key}`}>
                ...
                {free.length ? (
                  <div className="emp-list" role="list">
                    {free.map((e) => (
                      <div key={e.id} className="emp-row" role="listitem">
                        <div className="emp-name">{e.firstName} {e.lastName}</div>
                        <button
                          className="btn tiny"
                          onClick={() => handleAdd(e.id, d)}
                          aria-label={`Add ${e.firstName} ${e.lastName} on ${key}`}
                        >+ add</button>
                      </div>
                    ))}
                  </div>
                ...
                {/* Yard/Shop quick picker (single day UX) */}
                {selectedDate ? (
                  <div className="yard-card">
                    <div className="yard-title">Yard/Shop</div>
                    <div className="yard-row">
                      <select aria-label={`Select Yard/Shop employee for ${key}`} className="yard-select" defaultValue="">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          const val = sel?.value || '';
                          if (!val) return;
                          if (onSetYard) onSetYard(val, d);
                          else console.log('SetYard', val, key);
                        }}
                      >Set</button>
                    </div>
                    <div className="muted-xs">Creates an all-day "Yard/Shop" entry.</div>
                  </div>
                ) : null}
              </div>
            );
      ...
      <style jsx>{`
        ...
        .yard-card { margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px dashed var(--border); display: grid; gap: 0.35rem; }
        .yard-title { font-size: 0.85rem; color: var(--text-dim); font-weight: 600; }
