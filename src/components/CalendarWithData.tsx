...
export default function CalendarWithData({ calendarId, initialYear, initialMonth0 }: Props) {
  ...
  // drag/resize -> local update (server persistence for edit can be added later)
  ...
  const saveDraft = useCallback(async () => {
    ...
    if (editId) {
      // local edit; server PATCH can be added later
      setEvents(prev =>
        prev.map(ev =>
          ev.id === editId
            ? {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('gfc_todos');
      if (raw) setTodos(JSON.parse(raw));
    } catch {}
  }, []);
  useEffect(() => {
    try {
      localStorage.setItem('gfc_todos', JSON.stringify(todos));
    } catch {}
  }, [todos]);

  const addTodo = (type: JobType, title: string) => {
    const t = title.trim();
    if (!t) return;
    setTodos(p => [{ id: uid(), title: t, done: false, type }, ...p]);
  };
  ...
  const onDropToColumn = (e: React.DragEvent<HTMLDivElement>, type: JobType) => {
    e.preventDefault();
    const id = e.dataTransfer.getData('text/plain');
    if (id) moveTodo(id, type);
  };
          ...
          <div className="modal-card">
            ...
            <div className="form-grid form-compact">
              ...
              <div className="modal-actions span-2">
                {editId ? (
                  <>
                    <button className="btn" onClick={duplicateCurrent}>
                      Duplicate
                    </button>
                    <button className="btn ghost" onClick={deleteCurrent}>
                      Delete
                    </button>
                  </>
                ) : null}
                    ...
                    <div key={t.id} className="todo-card" draggable onDragStart={e => onDragStart(e, t.id)}>
                      ...
                      <div className="todo-actions">
                        <select
                          className="todo-move"
                          value={t.type}
                          onChange={e => moveTodo(t.id, e.target.value as JobType)}
                          title="Move to…"
                        >
                          {(['FENCE', 'GUARDRAIL', 'ATTENUATOR', 'HANDRAIL', 'TEMP_FENCE'] as JobType[]).map(v => (
                            <option key={v} value={v}>
                              {TYPE_LABEL[v]}
                            </option>
                          ))}
                        </select>
                        <button className="todo-del" onClick={() => deleteTodo(t.id)} title="Delete">
                          ×
                        </button>
                      </div>
                    </div>
...
function typeToClass(t?: NewEvent['type']) {
  switch (t) {
    case 'FENCE':
      return 'evt-fence';
    case 'TEMP_FENCE':
      return 'evt-temp-fence';
    case 'GUARDRAIL':
      return 'evt-guardrail';
    case 'HANDRAIL':
      return 'evt-handrail';
    case 'ATTENUATOR':
      return 'evt-attenuator';
    default:
      return '';
  }
}
...
function TodoAdder({ onAdd, placeholder }: { onAdd: (title: string) => void; placeholder: string }) {
  ...
  return (
    <div className="todo-adder">
      ...
      />
      <button className="btn primary todo-add-btn" onClick={submit}>
        Add
      </button>
    </div>
  );
