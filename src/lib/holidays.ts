...
async function fetchNager(year: number, cc: string): Promise<HolidayItem[]> {
  const res = await fetch(NAGER(year, cc), { next: { revalidate: 60 * 60 * 24 * 14 } })
  if (!res.ok) throw new Error(`Holiday fetch failed: ${res.status}`)
  const data: Array<{ date: string; localName: string; name: string; countryCode: string }> = await res.json()
  return data.map(h => ({
      ...
      p.$transaction(
        items.map(h =>
          p.holiday.upsert({
            ...
            create: { date: h.date, localName: h.localName, name: h.name, countryCode },
...
export async function getHolidaysDb(start: Date, end: Date, countryCode: string): Promise<Holiday[]> {
...
export async function ensureDbHydrated(start: Date, end: Date, country: string) {
...
export async function fetchHolidays(start: Date, end: Date, opts: { countryCode: string }) {
  try {
  ...
  return all
    ...
    .map(h => ({ date: h.date, localName: h.localName, name: h.name, countryCode: h.countryCode }))
}
